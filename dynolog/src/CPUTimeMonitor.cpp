...
// Returns min or max of a CPU breakdown metric over the time window.
// Breakdown values are already stored as per-second rates (cores) by
// processProcUsage(), so we pick min/max directly from the raw samples.
std::optional<double> CPUTimeMonitor::getCpuBreakdownMinOrMax(
    Granularity gran,
    uint64_t seconds_ago,
    CpuBreakdown breakdown,
    const std::optional<std::string>& targetId,
  ...
  return useMin ? *std::min_element(data.begin(), data.end())
...
std::optional<double> CPUTimeMonitor::getCpuBreakdownMin(
    ...
    CpuBreakdown breakdown,
    const std::optional<std::string>& targetId) {
  return getCpuBreakdownMinOrMax(gran, seconds_ago, breakdown, targetId, true);
}
...
std::optional<double> CPUTimeMonitor::getCpuBreakdownMax(
    ...
    CpuBreakdown breakdown,
    const std::optional<std::string>& targetId) {
  return getCpuBreakdownMinOrMax(gran, seconds_ago, breakdown, targetId, false);
}
